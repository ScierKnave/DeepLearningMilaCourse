...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        biased_var = torch.var(inputs, dim=-1, unbiased=False, keepdim=True)
        return (inputs - mean) / (torch.sqrt(biased_var) + self.eps) * self.weight + self.bias
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length=0):
        ...
        # TODO: Write your code here
        # ==========================
        self.W_Q = nn.Parameter(torch.empty(head_size*num_heads, head_size*num_heads))
        self.W_K = nn.Parameter(torch.empty(head_size*num_heads, head_size*num_heads))
        self.W_V = nn.Parameter(torch.empty(head_size*num_heads, head_size*num_heads))
        self.W_O = nn.Parameter(torch.empty(head_size*num_heads, head_size*num_heads))

        self.b_Q = nn.Parameter(torch.empty(head_size*num_heads))
        self.b_K = nn.Parameter(torch.empty(head_size*num_heads))
        self.b_V = nn.Parameter(torch.empty(head_size*num_heads))
        self.b_O = nn.Parameter(torch.empty(head_size*num_heads))

        for param in self.parameters():
            nn.init.uniform_(param, a=-(1/head_size*num_heads)**0.5, b=(1/head_size*num_heads)**0.5)
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        # TODO: Write your code here
        # ==========================
        weights = torch.matmul(queries, torch.transpose(keys, -1, -2)) / math.sqrt(self.head_size) # (B, N, S, S)
        if mask is not None: 
            weights = torch.masked_fill(weights, 
                                        ~mask.unsqueeze(1).unsqueeze(2).to(dtype=torch.bool), 
                                        float('-inf')) # (B, N, S, S)
        weights = F.softmax(weights, -1) # (B, N, S, S)
        return weights
    ...
    def apply_attention(self, queries, keys, values, mask=None):
        ...
        # TODO: Write your code here
        # ==========================
        weights = self.get_attention_weights(queries, keys, mask) # (B, N, S, S)
        attended_values = torch.matmul(weights, values) # (B, N, S, H)
        output = self.merge_heads(attended_values) # (B, S, E)
        return output  
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        B, S, E = tensor.shape # (B, S, E)
        tensor = torch.reshape(tensor, (B, S, self.num_heads, int(E/self.num_heads)))
    ...
    def merge_heads(self, tensor):
        ...
        tensor = torch.permute(tensor, (0, 2, 1, 3)) # (B, S, N, D)
        tensor = torch.reshape(tensor, (B, S, N*D))
        return tensor
    ...
    def forward(self, hidden_states, mask=None):
        ...
        Q = torch.matmul(hidden_states, self.W_Q) + self.b_Q
        K = torch.matmul(hidden_states, self.W_K) + self.b_K
        V = torch.matmul(hidden_states, self.W_V) + self.b_V
        Y = self.apply_attention(self.split_heads(Q), 
                                 self.split_heads(K), 
                                 self.split_heads(V),
                                 mask) # (B, S, E)
        outputs = torch.matmul(Y, self.W_O) + self.b_O
        return outputs
