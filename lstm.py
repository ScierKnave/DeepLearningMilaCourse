...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        # # ==========================
        B, S, I = inputs.shape 
        h, c = hidden_states # (1, B, H)
        h = torch.squeeze(h) # (B, H)
        c = torch.squeeze(c) # (B, H)
        y = torch.zeros(B, S, self.hidden_size) # (B, S, H)
        for s in range(S):
            x = torch.squeeze(inputs[:, s, :]) # (B, H)
            i = torch.sigmoid(x @ self.w_ii.t() + self.b_ii + h @ self.w_hi.t() + self.b_hi)
            f = torch.sigmoid(x @ self.w_if.t() + self.b_if + h @ self.w_hf.t() + self.b_hf)
            g = torch.tanh(   x @ self.w_ig.t() + self.b_ig + h @ self.w_hg.t() + self.b_hg) 
            o = torch.sigmoid(x @ self.w_io.t() + self.b_io + h @ self.w_ho.t() + self.b_ho)
            c = f * c + i * g
            h = o * torch.tanh(c)
            y[:, s, :] = h
...
class Encoder(nn.Module):
    def __init__(
        ...
        # TODO: Write your code here
        self.rnn = nn.LSTM(input_size=embedding_size, hidden_size=hidden_size, bidirectional=True, batch_first=True) 
    ...
    def forward(self, inputs, hidden_states):
        ...
        inputs = self.embedding(inputs) # (B, S, E)
        inputs = self.dropout(inputs) # (B, S, E)
        x, (h,c) = self.rnn(inputs, hidden_states) # (B, S, 2*H) and two (2, B, H) 
        #h = torch.unsqueeze(h, dim=0) # (1, B, 2*H)
        #h = torch.reshape(h, (2, -1, self.hidden_size)) # (2, B, H)
        #c = torch.unsqueeze(c, dim=0) # (1, B, 2*H)
        #c = torch.reshape(c, (2, -1, self.hidden_size)) # (2, B, H)
        return x, (h,c)
